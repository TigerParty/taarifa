...
<?php
			...
			foreach ($incidents as $incident)
			{
				...
				$strSqlCommand  = "SELECT * FROM incident WHERE incident_active = 1 AND id= ".$incident_id;
				$active = mysql_query($strSqlCommand);
				$rowActive = mysql_fetch_array ($active);

				$strSqlCommand2 = "SELECT * FROM GGroup WHERE `id`=".$rowActive['ggroup_id'];
				$resultGroup = mysql_query($strSqlCommand2);
				$rowGroup = mysql_fetch_array ($resultGroup);
				
				$GroupName = $rowGroup['GGroup_Name'];
				$GroupId = $rowGroup['id'];

				if(strcmp($rowActive['ggroup_id'],$pre_group_id)!=0){
					if(strcmp($rowActive['ggroup_id'],'0')!=0){
				...
				$pre_group_id = $rowActive['ggroup_id'];
...
?>
			...
			<div id="rb_map-view" style="display:none; width: 590px; height: 384px; border:1px solid #CCCCCC; margin: 3px auto;"></div>
			</div>
		<!-- /Report listing -->
