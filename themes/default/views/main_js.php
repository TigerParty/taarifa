    ...
    */    DrawChart();
    ...
    function DrawChart()
    {
        ...
        function(data)
        {
            ...
            {
                ChartData.addColumn('number', data.tittle.tittleX);
                ChartData.addColumn('number', data.tittle.tittleY);
                for( var i in data.data )
                {
                    ChartData.addRows([[parseInt(data.data[i].cate_name), parseInt(data.data[i].counter)]]);
                }
                ChartOptions = 
                {
                    title: $('#XAxis_selecter')[0].selectedOptions[0].innerText,
        			hAxis: {title: data.tittle.tittleX, minValue: data.tittle.rangeX.Min, maxValue: data.tittle.rangeX.Max},
		          	vAxis: {title: data.tittle.tittleY, minValue: data.tittle.rangeY.Min, maxValue: data.tittle.rangeY.Max},
                    'width':320,
                    'height':240
                };
           }
            ...
            {
                ChartData.addColumn('string', data.tittle.tittle);
                ChartData.addColumn('number', 'Count');
                for( var i in data.data )
                {
                    ChartData.addRows([[data.data[i].cate_name, parseInt(data.data[i].counter)]]);
                }
            }					
    ...
    function RedrawChart()
    {
        switch( $("#Chart_selecter").val() )
        {
            case "Column":
                ...
                {
                    'title': 'Taarifa',
                    'hAxis': {title: $('#XAxis_selecter')[0].selectedOptions[0].innerText, titleTextStyle: {color: 'black'}},
                    'vAxis': {title: $('#YAxis_selecter')[0].selectedOptions[0].innerText, titleTextStyle: {color: 'black'}},
                    'width':320,
                    'height':240
                };
                ChartCanvas = new google.visualization.ColumnChart(document.getElementById("chart_div"));
            ...
            case "Bar":
                ...
                {
                    title: 'Taarifa',
                    hAxis: {title: $('#YAxis_selecter')[0].selectedOptions[0].innerText, titleTextStyle: {color: 'black'}},
                    vAxis: {title: $('#XAxis_selecter')[0].selectedOptions[0].innerText, titleTextStyle: {color: 'black'}},
                    width: 320,
                    height: 240
                };
...
/*/
 		if( $("#Chart_selecter").val() != "Scatter" )
        {
            if( $("#XAxis_selecter").val() == -2 )
                ...
                var strSelect = '<option value="-2">Time</option><option value="-1">Category</option>';
			strSelect += '<option value="" disabled>----Data Field----</option>';
       	}
        else
        {
...
// TTP Plugin show all of subcategories as default
        	$("[id^='child_']").show(); // Hide All Children DIV
// End of TTP Plugin
...
//					$(this).parents("div").show();
					if( catID == 0 )noShiftParent = true;
                    else if( $("#child_"+catID).length == 1)noShiftParent = true;
                    else noShiftParent = false;
				}
                else
                {
                	noShiftParent = false;
                    ...
                    {
                    	$("#cat_0").removeClass("active"); // Remove "AllCategory"
                        if (markers && markers.length > 0 )
                        {
                            for (var i = 0; i < markers.length; i++)
                            {
                                // TTP Plugin	Remove Marker in AddMarker
                                if( arrayMarkersLayerID[ 0 ] == markers[i].id)
                                {
                                    map.removeLayer(markers[i]);
                                	arrayMarkersLayerID[ 0 ] = 0;
                                }
                            }
                        }
                    }
                    
                	if( catID == 0 )																			// All Categories
                    {
                    	isAddMarker = false;
                    }
                    else if( $("#child_"+catID).length == 1)													// Parent Categories
                    {
                    	if( $("#cat_" + catID).hasClass("active"))
                        {
                        	$("#cat_"+catID).removeClass("active");
                            // deSelect
                            if (markers && markers.length > 0 )
							{
								for (var i = 0; i < markers.length; i++)
								{
                                	// TTP Plugin	Remove Marker in AddMarker
                                    if( arrayMarkersLayerID[ catID ] == markers[i].id)
                                    {
                                    	map.removeLayer(markers[i]);
                                        arrayMarkersLayerID[ catID ] = 0;
                        ...
                        else if( $("#child_"+catID).css("display") == "block" )
                        {
                        	var ChildNode = $("#child_"+catID)[0].childNodes[0].childNodes;
                            var i;
                            for( i=0 ; i < ChildNode.length ; i++ )
                            {
                            	if(ChildNode[i].childNodes[0].className.indexOf("active") > -1)
                                {
                                	isAddMarker = false;
                                    break;
                                }
                            }
                            if( i == ChildNode.length) // no active child
                            {
	                        	$("#cat_" + catID).addClass("active"); // Add Highlight	
                            }
                        }
                    ...
                    {
                    	var ParentID = this.parentElement.parentElement.parentElement.id.substring(6);
                        if( $("#cat_"+ParentID).hasClass("active") )
                        {
                        	// undo
                                                    	    isAddMarker = false;
                        ...
                        else
                        {
	                		if( $("#cat_" + catID).hasClass("active") )
	                		{
                        		$("#cat_" + catID).removeClass("active"); // Add Highlight	
                        	    // deSelect
                        	    if (markers && markers.length > 0 )
								{
									for (var i = 0; i < markers.length; i++)
...
/*
                    for(var i = 1 ; i < $("a[id^='cat_']").length ; i++ )
                    {

                    	if( $("#cat_"+i).hasClass("active") )
                        {
                        	if( arrayMarkersLayerID[ i ] == 0)
                            {
                            ...
                            {
                            	if( !arrayMarkersLayerID[ i ] || arrayMarkersLayerID[ i ] == markers[j].id)
                                {
                                    arrayMarkersLayerID[ catID ] = 0;
									map.removeLayer(markers[i]);
                                }
...
*/				}
                ...
                $(this).parents("div").show();
// End of TTP Modify Category Click
