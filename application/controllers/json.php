...
class Json_Controller extends Template_Controller
{
	...
	public function index()
	{
		...
		$json_item_first = "";
		$incidentid_array = array();
		foreach ($markers as $marker)
		{
			$strSqlCommand = 'SELECT `ggroup_id` FROM `incident` where id = '.$marker->incident_id.' ORDER BY `incident_date` DESC';

			$result_groupid = $db->query($strSqlCommand);
			foreach ( $result_groupid as $item )
			{
				$incidentid_array[$marker->incident_id]=$item->ggroup_id;
			}
		}
		$array_final = array();
		foreach ($incidentid_array as $id => $id_group)
		{
			$id_exist = array_search($id_group, $array_final);
			if($id_exist==""||$id_group==0)
			{
				$array_final[$id]=$id_group;
			}
		}
		$array_final = array_keys($array_final);
		foreach ($markers as $marker)
		{
			
			if (in_array($marker->incident_id, $array_final)) {
				...
				if (!Category_Model::is_valid_category($category_id))
				{
					if($marker->email_send==0){
						...
						$color = "fafa00";
						$icon = "./media/img/email_ntf_icon.png";
					}
				}
