...
class Manage_Controller extends Admin_Controller
{
	...
	public function index()
	{
		...
		//select parent_id = 0 and parent's parent_id = 0					
		$parents_array = array();

		$strSqlCommand = "SELECT a.id, a.category_title FROM category as a 
		...
		while($row = mysql_fetch_array ($result)){
			$parents_array[$row['id']] = $row['category_title'];
		}
					...
					if ($category_id)
					{
						...
						if ($category->loaded)
						{
							...
							{ // ERROR!!!!!!!! WHY ARE YOU TRYING TO PLACE A SUBCATEGORY ABOVE A CATEGORY???
								...
								else if($category->parent_id != $parent_id){
									$category->parent_id = $sp_id;
								}
								else{
									if($parent_id)
									{
