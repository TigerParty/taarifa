...
<li><a href="javascript:delete_sub_category('<?php echo(rawurlencode($category_id)); ?>');catAction('d','DELETE','<?php echo(rawurlencode($category_id)); ?>');" class="del"><?php echo Kohana::lang('ui_main.delete');?></a></li>
												</ul>
	...
	<li><a href="javascript:delete_sub_category('<?php echo(rawurlencode($category_id)); ?>');catAction('d','DELETE','<?php echo(rawurlencode($category_id)); ?>');" class="del"><?php echo Kohana::lang('ui_main.delete');?></a></li>
											...
											$sql = "SELECT * FROM category WHERE parent_id =".$child->id.";";
											...
											if($results){
												while($result = mysql_fetch_array($results)){
													...
													$category_description = substr($result['category_description'], 0, 150);
													$category_color = $result['category_color'];
													$category_image = ($result['category_image'] != NULL) ? url::convert_uploaded_to_abs($result['category_image']) : NULL;
													$category_visible = $result['category_visible'];
...
<script>
	function delete_sub_category(id){
		$.ajax({
		  type: "GET",
		  url: "/Tphp/mail_act/deletecategory.php",
		  data: {
			   category_id:id
			  },
