						...
						<div class="f-col">
							...
							<div class="row">
			                    ...
			                    <div id="category_add" class="category_add">
                                    ...
                                    print '<br/>';
                                    print form::label("In category");
                                    print '<br/>';
									
                                    $strSqlCommand1 = "SELECT a.id,a.category_title FROM category as a 
                                    				   LEFT JOIN category as b 
                                    				   ON a.parent_id = b.id 
                                    				   WHERE b.parent_id = 0 OR a.parent_id = 0
                                    				   ORDER BY a.id ASC;";
                                    				   //$strSqlCommand1 = "SELECT * FROM category WHERE parent_id = 0;";
											$result = mysql_query($strSqlCommand1);
	  										echo "<select id='selectcategory' onChange='value = this.options[this.selectedIndex].value;'>";
                                    ...
                                    print '<br/>';
                                    print '<span>';
                                    print '<a href="#" onclick="InsertCategory()">'.Kohana::lang('ui_main.add').'</a>';
                                    print '</span>';

                                    ?> 
                                </div>

			                    <div class="report_category" id="category_display">
                        	    <?php
...
function InsertCategory(){
		...
		var category_selected = document.getElementById("selectcategory");
		var category_select = category_selected.options[category_selected.selectedIndex].value;
		...
		$.ajax({
			type: "GET",
			url: "/Tphp/mail_act/insertcategory.php",
			data: {
